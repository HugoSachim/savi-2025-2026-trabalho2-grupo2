...
class ModelFCN(nn.Module):
    def __init__(self, num_classes=11):
        ...
        # Camadas FCN (Substituem as FC)
        # kernel_size=3 aqui funciona como a primeira FC se a entrada for 3x3
        self.fcn_conv1 = nn.Conv2d(128, 128, kernel_size=3, padding=0) 
        
        # Camada Final: 128 -> 11 Classes (0-9 + Fundo)
        self.fcn_conv2 = nn.Conv2d(128, num_classes, kernel_size=1, padding=0)
    ...
    def forward(self, x):
        ...
        x = F.relu(self.bn4(self.conv4(x))) # Mantém 3x3
        x = self.dropout(x)

        # Cabeça FCN
        x = F.relu(self.fcn_conv1(x)) # 3x3 -> 1x1
        x = self.dropout(x)
        x = self.fcn_conv2(x)         # 1x1 -> 1x1 (Logits)
